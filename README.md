full rights go to https://github.com/Frosthaven/voicemeeter-windows-volume

modifications made:
- ported to c# (windows only)
- fixed the annoying tray icon dupe bug (happend on the js version because it tried to periodically check if vm isnt open, and if your vm crashed it will fail and reopen the app)
- one exe (selfcontained .net runtime), no installer 
- autorun registers over the registry now

<img width="290" height="407" alt="image" src="https://github.com/user-attachments/assets/77e06502-d52c-494e-a284-167e6616e69d" />

